...
typedef byte LTCFrame[10]; // 10 bytes = 80 bits = size of LTC frame per spec
...
/* in decoder mode, reset all values upon timer1 overflow, as this indicates a
loss of valid LTC signal */
ISR(TIMER1_OVF_vect)
{
    // TODO: does this need to be here? can't i enable this interrupt only in
    // decoder mode? so this fn could only be called if decoder mode 
    if (state == GENERATOR)
        return;
